# -*- coding: utf-8 -*-
"""
traffic_flow_analysis.py (Colab-compatible logic, but rewritten for local execution)
"""

# =========================
# Traffic Flow Analysis: Counting Vehicles from a YouTube Source
# Originally based on a Colab notebook, now adapted for local use
# =========================

# ---- SETTINGS ----
SOURCE_URL = "https://www.youtube.com/watch?v=MNn9qKG2UFI"
...
# from google.colab import files  # Removed—this module is not available when running locally

# ---- OPTIONAL: If you uncomment this, make sure to run in Colab environment ----
# files.download(os.path.join(...))
...
class CountingLineManager:
    ...
    def draw(self, frame):
        for idx, ln in enumerate(self.lanes, start=1):
            ...
            cv2.line(frame, (x-40,y), (x+40,y), (0,0,255), 3)
            cv2.putText(frame, f"Line {idx}", (x-30, y-12),
...
def download_youtube(url, out_path="input_video.mp4"):
    import yt_dlp
    with yt_dlp.YoutubeDL({'format': 'best[ext=mp4]/best',
                           'outtmpl': out_path,
                           'quiet': True}) as ydl:
...
def process_video(source, out_video, out_csv, model_name):
    ...
    with open(out_csv, 'w', newline='') as csv_f:
        ...
        csv_w.writerow(['vehicle_id', 'lane_number', 'frame', 'timestamp_s'])
        counted = set()
        ...
        while True:
            ...
            for tr in active_tracks:
                cx, cy = tr.centroid()
                lane_idx = next((idx for idx, ln in enumerate(lines.lanes, start=1)
                                  if ln['x_min'] <= cx <= ln['x_max']), None)
                x1, y1, x2, y2 = map(int, tr.bbox)
                color = (0, 255, 0) if (tr.track_id, lane_idx) in counted else (0, 165, 255)
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                cv2.putText(frame, f"ID {tr.track_id}", (x1, y1 - 8),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

                if lane_idx is not None and tr.prev_centroid is not None:
                    prev_y = tr.prev_centroid[1]
                    line_y = lines.lanes[lane_idx - 1]['line_y']
                    crossed = ((prev_y < line_y <= cy) if not COUNT_BOTH_DIRECTIONS
                               else ((prev_y < line_y <= cy) or (prev_y > line_y >= cy)))
            ...
            for i in range(1, 4):
                cv2.putText(frame, f"Lane {i}: {per_lane_counts[i]}",
                            (10, 30 + 30 * i), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
